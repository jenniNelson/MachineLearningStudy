...
def recover_b_from_alphas_only(a, x, y, threshold, kernel=np.dot):
    b = 0
    num_nonzero = 0
    for i in range(len(a)):
        if a[i] > threshold:
            ...
            for j in range(len(a)):
                inner_sum += a[j] * y[j] * kernel(x[j], x[i])
            b+= y[i] - inner_sum
            num_nonzero += 1
    return b/num_nonzero
...
def kernel_predict(a, x, y, kernel, sample):
    ...
    b = recover_b_from_alphas_only(a, x,y, 1e-11, kernel)
...
if __name__ == '__main__':
    ...
    for C in [100/873, 500/873, 700/873]:
        ...
        train_err = average_error(banknote_train_examples,
                                  lambda sample: predict_standard(sample, incorporated_weights))
        test_err = average_error(banknote_train_examples,
                                 lambda sample: predict_standard(sample, incorporated_weights))
        other_train_err = average_error(banknote_train_examples,
                                  lambda sample: kernel_predict(alphas, train_vals[:max_num_samples], train_labels[:max_num_samples], np.dot, sample.attributes[:-1]))
        other_test_err = average_error(banknote_train_examples,
                             lambda sample: kernel_predict(alphas, train_vals[:max_num_samples], train_labels[:max_num_samples], np.dot, sample.attributes[:-1]))
    ...
    format_string = "{:<7.3f}\t&{:<7.3f}\t&{:>10.3f}\t&{:>10.3f}\t&{:>10.0f}\t\\\\\hline"

    for gamma in [0.01, 0.1, 0.5, 1, 2, 5, 10, 100]:
       for C in [100/873, 500/873, 700/873]:
            ...
            print(format_string.format(C, gamma, train_err, test_err, count_svm(alphas)))
    ...
    for gamma in [0.01, 0.1, 0.5, 1, 2, 5, 10, 100]:
        C = 500/873
        alphas = dual_svm(train_vals[:max_num_samples], train_labels[:max_num_samples], C,
                          lambda w, x: gaussian_kernel(w, x, gamma))
        new_sv = list()
        num_overlap = 0
        ...
        print("{:<7.3f}\t&{:>10.0f}\t\\\\\hline".format(gamma, num_overlap))
        old_sv = new_sv
